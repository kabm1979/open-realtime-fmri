...
/* parse one line and add to the list pointed to by "*first" 
	...
	added "expect double quotes" option as double quotes used in shadowdata file
*/
...
sap_item_t *sap_parse(const char *buffer, unsigned int length) {
	...
	// Rhodri Cusack, Mar 2010 - if this is an SV10 file, skip everything up to ### ASCCONV BEGIN ###, and replace double quotes with single
	char svheader[5];
	strcpy(svheader,"SV10");
	issvn10=-1;
	for(i=0; i<4; i++) if (svheader[i]!=buffer[i]) issvn10=0;
	if (issvn10) {
		char ascstr[24];
		strcpy(ascstr,"### ASCCONV BEGIN ###");
		// Rhodri end
		
		match=0;
		// Rhodri Cusack, Mar 2010 - skip everything up to ### ASCCONV BEGIN ###
		while (!match && (length>0)) {
			for (nextLine = curLine; *nextLine!='\n'; nextLine++) {
				if (--length == 0) return first;
			}
			if (strlen(ascstr)==(nextLine-curLine))
			{
				match=1;
				for (i=0; i<strlen(ascstr); i++)
					if (ascstr[i]!=curLine[i])
						match=0;
			};
			/* skip the \n for next time */
			curLine=nextLine+1;
	...
	while (length>0) {
		...
		if (issvn10) {
			c2=curLine;
			for(c1=curLine; c1<nextLine; c1++)
			{
				*(c2++)=*(c1);
				if ((*c1)=='"') c1++;
			};
		};		
		sap_handle_line(&first, curLine, c2);
		/* skip the \n for next time */
		curLine=nextLine+1;
		length--;
	} 
