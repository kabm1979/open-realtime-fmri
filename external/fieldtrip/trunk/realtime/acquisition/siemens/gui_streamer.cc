...
int main(int argc, char *argv[]) {
	...
	Fl::visual(FL_RGB);
	window = new Fl_Window(100,100,400,350,"Realtime streamer (rhodri-1.03)");
		...
		if (pdg.run(20) < 0) {
			...
			switch(pdg.getLastAction()) {
				...
				case PixelDataGrabber::PixelsTransmitted:
					getNumWrittenSinceLastReport=pdg.getNumWrittenSinceLastReport();

					if (getNumWrittenSinceLastReport==pdg.getNumWritten()) {
						char msg[256];
						sprintf(msg,"Transmitted first %d sample[s]",getNumWrittenSinceLastReport);
						addTimedMsg(msg);
					}
					else
					{
						char msg[256];
						sprintf(msg,"Transmitted further %d sample[s]",getNumWrittenSinceLastReport);
						addTimedMsg(msg);
					};
					updatePiBox();
					break;
